    ...
    def _encode_prompt_data_example(
        ...
        if last_user is None:
            fill_id = (
                ...
                else (self.tokenizer.pad_token_id if self.tokenizer.pad_token_id is not None else 0)
            )
            return [fill_id]
        ...
        # 关键：用 chat_template 并开启 add_generation_prompt
        ...
        ids, _ = self.template.mm_plugin.process_token_ids(
            ids, None, images, videos, audios, self.tokenizer, self.processor
        )

        # 截断
        source_len = min(self.data_args.cutoff_len, len(ids))
        return ids[:source_len]
