...
class SFTDataCollatorWith4DAttentionMask(MultiModalDataCollatorForSeq2Seq):
    ...
    def __call__(self, features: list[dict[str, Any]]) -> dict[str, "torch.Tensor"]:
            lan_list = ['en', 'zh', 'ru', 'ja', 'bn', 'sw', 'ur', 'ps', 'ku', 'ar']
            has_multilingual = any(
                any(k.startswith("prompt_input_ids_") for k in feature.keys()) 
                for feature in features
            )
            ...
            for key, value in batch.items():  # cast data dtype for paligemma
                if torch.is_tensor(value) and torch.is_floating_point(value):
                    batch[key] = value.to(self.compute_dtype)

            if has_multilingual:
                for lang in lan_list:
                    prompt_ids_key = f"prompt_input_ids_{lang}"
                    prompt_mask_key = f"prompt_attention_mask_{lang}"
                        
                    if prompt_ids_key in batch:
                        batch[prompt_ids_key] = batch[prompt_ids_key].to(self.compute_dtype)
                    if prompt_mask_key in batch:
                        batch[prompt_mask_key] = batch[prompt_mask_key].to(self.compute_dtype)

                    # print("batch prompt_ids_key:\n",batch[prompt_ids_key].shape)
                    # print("batch prompt_mask_key:\n",batch[prompt_mask_key].shape)
            
            batch['num_pairs'] = torch.tensor(len(features), dtype=torch.long)
...
class PairwiseDataCollatorWithPadding(MultiModalDataCollatorForSeq2Seq):
    ...
    def __call__(self, features: list[dict[str, Any]]) -> dict[str, "torch.Tensor"]:
        ...
        # lan_list=['en','zh','ru','ja','vi','ko','fr','ar','es','it','ur']
        lan_list=['en','zh','ru','ja','bn','sw','ur','ps','ku','ar']
        if any(any(k.startswith("prompt_input_ids_") for k in f.keys()) for f in features):
            first = features[0]
            lan_list = sorted({
                k.split("prompt_input_ids_")[1]
                for k in first.keys() if k.startswith("prompt_input_ids_")
            })
            for f in features:
                for lan in lan_list:
                    pid = f.get(f"prompt_input_ids_{lan}")
                    pmask = f.get(f"prompt_attention_mask_{lan}")
                    if pid is None or pmask is None:
                        continue
                    concatenated_features.append({
                        "input_ids": pid,
                        "attention_mask": pmask,
                        "labels": [IGNORE_INDEX] * len(pid),
                        "images": f.get("images"),
                        "videos": f.get("videos"),
                        "audios": f.get("audios"),
                    })
